...
class PeerMetadata(object):
    ...
    def __str__(self):
        return "{} {}".format(self.conn_addr, self.is_stable)


class PeerManager(object):
    TIMEOUT_SECS = 2
    REFRESH_CYCLE_SECS = 4
    ...
    def add(self, addr_list):
        ...
        with self._lock:
            ...
            for new_peer_ip in addr_list:
                if new_peer_ip not in ['127.0.0.1', '0', '0.0.0.0']:
        ...
        while True:
            try:
                for peer_metadata in self._all_peers():
                    stub = peer_metadata.stub
                    if peer_metadata.peers_needs_refresh:
                        f = stub.GetKnownPeers.future(qrl_pb2.GetKnownPeersReq(), timeout=self.TIMEOUT_SECS)
                        f.pm = peer_metadata
                        f.add_done_callback(self._add_peers_callback)
                    else:
                        f = stub.GetNodeState.future(qrl_pb2.GetNodeStateReq(), timeout=self.TIMEOUT_SECS)
                        f.pm = peer_metadata
