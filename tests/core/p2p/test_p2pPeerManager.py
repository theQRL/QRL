...
class TestP2PPeerManager(TestCase):
    ...
    def test_monitor_chain_state_stale_info(self):
        ...
        """
        channel_1, channel_2, channel_3 = self.populate_peer_manager()
        self.peer_manager._peer_node_status[channel_1].timestamp = int(time.time() - config.user.chain_state_timeout)
    ...
    def test_handle_chain_state_works(self):
        ...
        # The timestamp is updated. But we can't check that reliably without slowing the test down.
        # Instead, let's check that the chain_state_data we passed to the handle_chain_state() is now in
        # peer_node_status. It is only a reference, so it really should pass.
        self.assertEqual(self.peer_manager._peer_node_status[channel], chain_state_data)
