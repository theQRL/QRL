...
def list_addresses():
	...
	for address in addr:
		...
		if type(address[1]) == list:	
			...
			list_addr.append([address[0], 'type:', address[1].type, 'balance: '+str(chain.state_balance(address[0]))+'('+str(chain.state_balance(address[0])+x)+')', 'nonce:'+str(chain.state_nonce(address[0]))+'('+str(chain.state_nonce(address[0])+y)+')', 'remaining signatures: '+str(address[1].remaining)+' ('+str(address[1].remaining)+'/'+str(address[1].signatures)+')'])
